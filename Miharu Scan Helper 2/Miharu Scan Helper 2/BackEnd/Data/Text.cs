...
namespace Miharu2.BackEnd.Data
{
	...
	public class Text
    {
		...
		public Text (Image src, DPIAwareRectangle rect) {
			Uuid = Guid.NewGuid();
		...
		public Text (Guid? uuid,
			...
			if (dpiAwareRectangle != null)
				...
				double dpi = Eto.Forms.Screen.PrimaryScreen.RealDPI;
				DpiAwareRectangle = new DPIAwareRectangle(rectangle, dpi, dpi);
			}
			Vertical = vertical;
		...
		private string ParseText () {
			...
			Source.SaveAsPng(TEMP_IMG);

			using (Process pProcess = new Process()) {
				pProcess.StartInfo.FileName = Settings.Get<string>("TesseractPath");
